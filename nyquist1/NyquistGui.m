...
axes(handles.sDomAx);
title('s-domain plot (w/ poles and zeros of F(s))');
ylabel('imag(s)');  xlabel('real(s)');

myGrid(handles.ghDomAx, handles);  %Initialize axes
ylabel('imag( F(s) )');  xlabel('real( F(s) )');
if get(handles.NyqPathRButt,'Value'),
    title('F(s)-domain plot (w/ the point -1 shown)); F(s)=G(s)H(s)');
...
s={ '* Time delays are ignored.  (You can add them by altering the',...
    ...
    '* The drawing of the angle subtended by the path in "F(s)"',...
...
if i==maxlen,
    warndlg('Large changes in F(s), plot inaccurate.');
end
ondet=ondet(1:i);
...
N=Z-P;
s1=['The open-loop transfer function, F(s), has P=' num2str(P)...
    ' pole(s) in the RHP.'];
...
s5=' ';
if sum(p_onax)~=0,
    s6=['"F(s)" path goes through -1+j0, so there are some closed ',...
        'loop poles on jw axis.'];
    s7='The angles drawn on the "F(s)" plot may be inaccurate!'; 
    s={s1;s2;s3;s4;s5;s6;s7};
else
    s={s1;s2;s3;s4};
end
