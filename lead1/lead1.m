...
% set(v2line,'LineStyle','--')
%%
set(ax(3),'YLim', [-40 40]) % Set mag axis
% set(ax(3),'YLim', [-40 40],'YTick',[-100 -80 -60 -40 -18.5 -7.3 0 20 40 60 80]) % Set mag axis
set(ax(3),'XLim', [1 100]) % Set mag axis
set(ax(2),'XLim', [1 100])  % Set phase axis
% set(ax(2),'YLim', [-180 -90])  % Set phase axis
set(ax(2),'YLim', [-180 -90],'YTick',[-180 -162 -150 -129.6 -120 -90])  % Set phase axis
newXTicks=ax(3).XTick;
newXTicks(end+1)=6.17;
newXTicks(end+1)=8.94;
% newXTicks(end+1)=.242;
newXTicks = sort(newXTicks);
set(ax(2),'XTick',newXTicks) % Set mag axis
set(ax(3),'XTick',newXTicks) % Set mag axis

text(ax(3),1.1, 13.5, 'Uncompensated $\nearrow$', 'Interpreter', 'latex')
text(ax(3),2, 20, '$\swarrow$ Compensated ', 'Interpreter', 'latex')
...
legend('Gp(s)=4K/(s(s+2))','Gp(s)Gc(s)=(4.2(40)(s+4.41))/(s(s+2)(s+18.4))')
